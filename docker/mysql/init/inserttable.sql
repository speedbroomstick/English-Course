...
INSERT INTO `rule` (name,title,ruleText,test_id) VALUES ('Артикли','Артикли a,an','Артикль "a" используется перед существительными, которые начинаются с согласного звука, например: "a dog" (собака), "a car" (машина), "a house" (дом).
...
Запомните, что выбор артикля зависит от произношения слова, а не от его написания. Например, перед словом "university" следует использовать "an" (an university), потому что оно произносится с гласным звуком в начале.',1),
...
INSERT INTO `questionForTest` (question,answer,type_i,type_oi,test_id) VALUES ('Артикли a,an','','rule','rule',1),
